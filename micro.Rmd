...
```{r}
control_analysis <- function(clrlmms,i=5,ctrl=6){ 
  ...
  df_times <- c(clrlmms[[i]]$time, clrlmms[[ctrl]]$time)
  
  #raw non-lmms raw clr data for each mouse
  df_all_control_F<- rbind(clrlmms[[i]]$data.filtered, clrlmms[[ctrl]]$data.filtered) 
  ...
  df_group <- c(replicate(length(clrlmms[[i]]$time),"G1"), 
  ...
  col_bind <- rbind(clrlmms[[i]]$data.filtered, clrlmms[[ctrl]]$data.filtered) %>% 
    ...
    add_column(Time = c(clrlmms[[i]]$time, clrlmms[[ctrl]]$time), .before = 1)
  ...
  p <- ggplot() +
        ...
        ggtitle(paste0("OTUs of Group Difference LMMSDE: ", 
                       clrlmms[[i]]$antibiotic," and ", 
                       clrlmms[[ctrl]]$antibiotic ))  
  return(p)
}

control_analysis(mouse_results,i=5,ctrl=6)
```
```{r}
plot_networks <- function(clrlmms, exp=5, ctrl=6, proportion=FALSE){ 
  ...
  for (i in c(1,2)) {
    ...
    plot(g, edge.width = E(g)$width, 
         main= vector[[i]]$anti,
         mode = "undirected",
         vertex.frame.color = NA, vertex.size = 15, 
