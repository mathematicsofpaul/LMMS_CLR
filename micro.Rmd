...
multipurpose <- function(mouse_id_antibiotic, df_vanco=df_vanco, df_taxa2=df_taxa2, lowdays=lowdays) {
  ...
  for (mouse in mouse_id_antibiotic) {
    ...
    #antibiotic name finder 
    ant <- df_vanco %>% filter(host_subject_id==mouse) %>% ungroup() %>% 
      dplyr::select(antibiotic) %>% dplyr::first() %>% dplyr::first()
  ...
  DATA$antibiotic <- ant #antibiotics
...
mouse_control <- mice_names[[6]] # print the list mice_control to know the 
...
```
## Work in progress for control analysis
```{r}
control_analysis <- function(i=5,j=6){ 
  i=5 
  j=6
  df_times <- c(ult[[i]]$timey,ult[[j]]$timey)
  
  df_all_control_F<- rbind(ult[[i]]$data.filtered, ult[[j]]$data.filtered) 
  #sample names 
  df_de_names <- row.names(df_all_control_F)
  
  #Groups 
  df_group <- c(replicate(length(ult[[i]]$timey),"G1"),replicate(length(ult[[j]]$timey),"G2")) 
  ...
  myvars <- paste("G1", ult[[5]]$timey, sep=" ")
  myvars2 <- paste("G2",ult[[6]]$timey, sep=" ") 
  
  meh <- lmmsDEtest2@DE %>% filter(adj.Group<0.05) %>% dplyr::select(Molecule) %>% 
    lapply(function(x) as.numeric(gsub("OTU", "", x))) 
  
  #takes the middle 10 from 20 to 30.
  otUs <- meh$Molecule[c(20,22)]
  
  group_1 <-lmmsDEtest2@predGroup[,myvars][otUs,] %>% as.data.frame() %>%
    add_column(OTUs = otUs, .before = 1) %>%
    add_column(Group = rep("Experimental", times=length(otUs)), .before = 2) 
  
  group_1 <- pivot_longer(group_1, -c("Group", "OTUs"), values_to = "Value", names_to = "Time")
  group_1
  
  group_2 <-lmmsDEtest2@predGroup[,myvars2][otUs,] %>% as.data.frame() %>% 
    add_column(OTUs = otUs, .before = 1) %>% 
    add_column(Group = rep("Control", times=length(otUs)), .before = 2) 
  
  group_2 <-pivot_longer(group_2, -c("Group", "OTUs"), values_to = "Value", names_to = "Time")
  group_2 
  
  super_frame <- rbind(group_1,group_2) 
  super_frame$Time <- rep(ult[[6]]$time, times=2*length(otUs))
  
  col_bind<-cbind(df_present$host_subject_id, df_present$sample_id,
                  df_present$day_of_experiment_numerical,
                  df_all_clr) %>% 
    add_column(Group = c(rep("Experimental", times=3*33), rep("Control", times=3*33)), .before = 1) 
  
  col_bind %>% View()                  
  
  other.frame <- col_bind %>% dplyr::select(-c("df_total$host_subject_id","df_total$sample_id")) %>%  
    pivot_longer(-c("Group", "df_total$day_of_experiment_numerical"), values_to = "Value", names_to = "OTUs") %>% 
    mutate(OTUs = as.numeric(gsub("OTU", "", OTUs))) %>% filter(OTUs %in% otUs) %>% 
    rename("df_total$day_of_experiment_numerical"="Time")
  
  ggplot() +
    geom_line(data = super_frame, aes(x = Time, y = Value, color = Group)) + 
    geom_point(data = other.frame, aes(x = Time, y = Value, color = Group)) +
    facet_wrap(facets =  vars(OTUs)) + 
    ggtitle("OTUs of Group Difference LMMSDE")
  
  #lmmsDEtest@modelsUsed
  return(lmmsDEtest)
  }
```
